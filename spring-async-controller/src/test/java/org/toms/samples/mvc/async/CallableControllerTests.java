package org.toms.samples.mvc.async;
...
import org.toms.samples.mvc.AbstractContextControllerTests;
...
public class CallableControllerTests extends AbstractContextControllerTests {
	...
	public void view() throws Exception {
		MvcResult mvcResult = this.mockMvc.perform(get("/async/callable/view/DD001"))
			.andExpect(request().asyncStarted())
			.andExpect(request().asyncResult("html"))
			.andReturn();

		this.mockMvc.perform(asyncDispatch(mvcResult))
			.andExpect(status().isOk())
			.andExpect(forwardedUrl("/WEB-INF/views/html.jsp"))
			.andExpect(model().attribute("name", "bar"))
			.andExpect(model().attribute("message", "apple"));
	}
